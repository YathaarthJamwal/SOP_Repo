...
# binary_feature_matrix = [ [random.randint(0,1) for i in range(columns)] for j in range(rows) ]
# sum_of_squares_template = 0
# for i in range(rows):
#     for j in range(columns):
#         sum_of_squares_template += binary_feature_matrix[i][j]
# encrypted_sum_squares = encrypt(pub, sum_of_squares_template)

# encrypted_feature_matrix = [ [encrypt(pub, binary_feature_matrix[i][j]) for j in range(columns)] for i in range(rows) ]

# matrix2 = [ [j for j in binary_feature_matrix[i]] for i  in range(len(binary_feature_matrix)) ]
# matrix2[0][0] = (1 if binary_feature_matrix[0][0] == 0 else 0)
# sum_squares_2 = 0
# for i in range(rows):
#     for j in range(columns):
#         sum_squares_2 += matrix2[i][j]
# encrypted_sum_squares = encrypt(pub, sum_squares_2)

# # DISTANCE CALCULATION BETWEEN ENCRYPTED TEMPLATES
# encrypted_distances = [0 for i in range(1)]
# for i in range(1):
#     encrypted_distances[i] = 
