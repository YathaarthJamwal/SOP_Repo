...
# inverse_encrypted_length = invmod(encrypted_length, pub.n_sq)
...
class Server:
    ...
    def calculateValues(self):
        ...
        self.z_encrypted = e_add(self.pubkey, encrypt(self.pubkey, 2**length), e_add(self.pubkey, self.X_encrypted, invmod(self.Y_encrypted, self.pubkey.n_sq)))
    ...
    def final_calculation(self, encrypted_lbda, d_cap_encrypted):
        ...
        d_cap_encrypted_inverse = invmod(d_cap_encrypted, self.pubkey.n_sq)

        # encrypted_lbda_powered = e_mul_const(self.pubkey, invmod(encrypted_lbda, self.pubkey.n_sq), 2**32)
        encrypted_lbda_powered = invmod(e_mul_const(self.pubkey, encrypted_lbda, 2**length), self.pubkey.n_sq)
...
print(decrypt(priv, pub, b_encrypted))
